...
def list_sort_nicely(l):
    ...
    return l
def generate_JSON(path_originalData, model_dict, label_dict, target):
...
if __name__ == "__main__":
    path_originalData = "./datasets/Task66_liver/"
    os.makedirs(path_originalData, exist_ok=True)
    os.makedirs(path_originalData + "imagesTr/", exist_ok=True)
    os.makedirs(path_originalData + "labelsTr/", exist_ok=True)
    os.makedirs(path_originalData + "imagesTs/", exist_ok=True)
    os.makedirs(path_originalData + "labelsTs/", exist_ok=True)
    model_dict = {
        "0": "CT"
    }
    label_dict = {
    ...
    target = 'liver'
    generate_JSON(path_originalData, model_dict, label_dict, target)
